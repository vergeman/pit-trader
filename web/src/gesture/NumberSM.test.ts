...
describe("NumberSM", () => {
  ...
  it("update() returns sum of descending gesture values", async () => {
    const gesture100 = new Gesture(GestureType.Qty, GestureAction.Buy, 100);
    const gesture70 = new Gesture(GestureType.Qty, GestureAction.Buy, 70);
    const gesture3 = new Gesture(GestureType.Qty, GestureAction.Buy, 3);
    const TIMEOUT = 100;
    const onFinalTimeout = jest.fn();
    const qtySM = new NumberSM(GestureType.Qty, onFinalTimeout, TIMEOUT);
  ...
  it("update() returns 'replaced' gesture value with same digit length with single callback", async () => {
    const gesture100 = new Gesture(GestureType.Qty, GestureAction.Buy, 100);
    const gesture70 = new Gesture(GestureType.Qty, GestureAction.Buy, 70);
    const gesture80 = new Gesture(GestureType.Qty, GestureAction.Buy, 80);
    const TIMEOUT = 100;
    const onFinalTimeout = jest.fn();
    const qtySM = new NumberSM(GestureType.Qty, onFinalTimeout, TIMEOUT);
